...
class Parser {
    ...
    Product parseProduct(String url) throws IOException {
        ...
        categoryID = listener.getCategoryID(categoryName);
        if (categoryID == 0) {
    ...
    //TODO не совсем валидно сразу инсёртить продукт, возможно, его следует передать наверх через сервисы
    void insertProductToDB(Product product) {
        listener.insertProduct(product.getName(), product.getURL(), product.getPrice(), product.getCategoryID(), product.getGroup().getName(), product.getVolume(), product.getStrength());
    }
}